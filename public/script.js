...
const search = async () => {
  try {
    ...
    let result = fetch(`https://api.travelpayouts.com/aviasales/v3/prices_for_dates?origin=${originInput.getAttribute('flightData')}&destination=${destinationInput.getAttribute('flightData1')}&unique=false&sorting=price&direct=false&currency=usd&limit=30&page=1&token=aebed97e88519a5cb765d5c68866a86b`, { method: "GET", mode: 'cors', headers: { 'Content-Type': 'application/json',}}).then(response => response.json())
